...
class SX1276:
    ...
    def spi_write(self, reg:bytes, data:str, fifo:bool=False):
        """
        The `spi_write` function writes data to a specified register using SPI communication.
        
        :param reg: The "reg" parameter is the register address that you want to write to. It is used to
        determine the specific register in the device that you want to write data to
        :param data: The "data" parameter is the value that you want to write to a specific register. It can
        be a single byte or a sequence of bytes, depending on the register you are writing to
        :param fifo: The `fifo` parameter is a boolean flag that determines whether the data should be
        written to the FIFO (First-In-First-Out) buffer or not. If `fifo` is set to `True`, the data will be
        written to the FIFO buffer. If `fifo` is set to `False, defaults to False (optional)
        """
    ...
    def spi_read(self, reg:bytes=None, length:int=None) -> bytes:
        """
        The `spi_read` function reads data from a SPI device using the specified register and length.
        
        :param reg: The `reg` parameter is a bytes object that represents the register address from which
        you want to read data. It is an optional parameter, meaning you can omit it if you don't need to
        specify a specific register
        :type reg: bytes
        :param length: The `length` parameter is an integer that specifies the number of bytes to read from
        the SPI device. If `length` is not provided or is set to `None`, the function will read 2 bytes from
        the SPI device
        :type length: int
        :return: The `spi_read` function returns the data read from the SPI device.
        """
        self.cs_pin.value(0)
    ...
    def set_freq(self) -> None:
        """
        The `set_freq` function sets the frequency of a radio module based on the current channel.
        """
    ...
    def read_fifo(self):
        """
        The function `read_fifo` reads data from a FIFO register, calculates the Signal-to-Noise Ratio (SNR)
        and Received Signal Strength Indicator (RSSI), and returns the packet, SNR, and RSSI values.
        :return: three values: 'packet', 'SNR', and 'RSSI'.
        """
    ...
    def write_fifo(self, data:bytes) -> None:
        """
        The function writes data to a FIFO buffer using SPI communication.
        
        :param data: The `data` parameter is of type `bytes` and represents the data that needs to be
        written to the FIFO (First-In-First-Out) buffer
        :type data: bytes
        """
    ...
    def send(self, dst_id:int=0, pkt_id:int=0, pkt_type:int=0, msg:str='', retry:int=1, timeout:int=9, debug:bool=False) -> None: # src_id, dst_id,
        """
        The `send` function is responsible for sending packets with different types of messages, handling
        retries and timeouts, and managing the mode of the modem.
        
        :param dst_id: The `dst_id` parameter is an integer that represents the destination ID of the
        packet. It specifies the ID of the device to which the packet is being sent, defaults to 0
        :type dst_id: int (optional)
        :param pkt_id: The `pkt_id` parameter is an integer that represents the packet ID. It is used to
        uniquely identify a packet, defaults to 0
        :type pkt_id: int (optional)
        :param pkt_type: The `pkt_type` parameter is an integer that represents the type of packet being
        sent. It can take on the following values:, defaults to 0
        :type pkt_type: int (optional)
        :param msg: The `msg` parameter is a string that represents the message you want to send. It will be
        encoded and included in the payload of the packet that is sent
        :type msg: str
        :param retry: The `retry` parameter specifies the number of times the packet should be resent if it
        is not acknowledged. If the packet type is a request (`PKT_TYPE['REQ']`), the code will attempt to
        resend the packet `retry` number of times before giving up, defaults to 1
        :type retry: int (optional)
        :param timeout: The `timeout` parameter in the `send` method specifies the amount of time (in
        seconds) that the program will wait for a response after sending a request packet. If no response is
        received within the specified timeout period, the program will consider the request as not
        acknowledged, defaults to 9
        :type timeout: int (optional)
        :param debug: The `debug` parameter is a boolean flag that determines whether or not to print debug
        messages during the execution of the `send` method. If `debug` is set to `True`, debug messages will
        be printed. If `debug` is set to `False`, no debug messages will be printed, defaults to False
    ...
    def _irq_handler(self, pin:int):
        """
        The `_irq_handler` function handles different types of interrupts and performs specific actions
        based on the type of interrupt received.
        
        :param pin: The `pin` parameter is an integer that represents the pin number that triggered the
        interrupt
        :type pin: int
